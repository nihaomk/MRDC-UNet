...
#####
    ...
    for ind in range(len(gt_imgs)):
        ...
        if len(label.flatten()) != len(pred.flatten()):
            ...
            continue
        ###mean dice
        dice_score = dice_coef(pred, label)
        ...
        if name_classes is not None and ind > 0 and ind % 10 == 0:
            print('{:d} / {:d}: mIou-{:0.4f}%; mPA-{:0.4f}%; Accuracy-{:0.4f}%'.format(
                ...
                #100* per_class_iu(hist)
            )
            )
##########
    mean_dice = total_dice / len(gt_imgs)
    # 打印平均Dice系数
    print('===> mDice: ' + str(round(mean_dice * 100, 4)))
#########
    # ------------------------------------------------#
...
def dice_coef(y1, y2):
    intersection = np.sum(y1 * y2)
    return (2. * intersection) / (np.sum(y1) + np.sum(y2))
def main():
    ...
    for file in file_name:
        ...
        png_name_list2.append(numeric_part + ".png")
        #png_name_list2.append(numeric_part)
        ########################
        png_name_list.append(file)

        # The path of logs
    log_path = args.log_path

    # Log file is named based on the running time
    log_file_name = log_path + 'log-' + 'MRDCUNet' + '.log'
     # save print message
    sys.stdout = Logger(log_file_name, stream=sys.stdout)
    compute_mIoU(gt_dir, pred_dir, png_name_list, png_name_list2, num_classes=2, name_classes=["_background_", "Tactile_paving"])
    ### mean Dice
...
def parse_args():
    ...
    parser.add_argument("--gt_dir", default="./data/",help="The root of Dataset ground truth list file")
    parser.add_argument("--txt_dir", default="predict.txt", help="The root of predicted file")
    parser.add_argument("--pred_dir", default="./predict/ClinicDB", help="The root of predicted results of testing samples")
